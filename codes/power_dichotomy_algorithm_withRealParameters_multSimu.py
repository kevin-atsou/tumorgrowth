...
np.savetxt("power_dicho/mu1_vs_{0}.txt".format(params.tested_param), mus)


# -------------------------------------- Result visualization -----------------------------------

Y = np.loadtxt("power_dicho/mu1_vs_{0}.txt".format(params.tested_param))
...
fig = plt.figure(3)
size_of_fig=8
fig.set_size_inches(size_of_fig+7, size_of_fig+2)

color1 = 'tab:red'
# plt.plot(tested_vars, Y, '-', color='k', linewidth=4, label='$t \mapsto \mu_1(t)$')
# plt.plot(S_s, Y, '-', color='k', linewidth=4, label='$t \mapsto \mu_1(t)$')
plt.plot(tested_vars, Y, '-', color='k', linewidth=4, label='$t \mapsto \mu_1(t)$')
plt.ylabel('tumor diameter in $mm$')
# plt.xlabel('division rate $a$ in $day^{-1}$')
# plt.text(np.max(tested_vars) - 5*(tested_vars[-1] - tested_vars[-2]), np.max(Y), '$A={0}$'.format(params.delta))
...
# plt.xlabel('Im. cells Diff. $D$ in $ mm^{2} \cdot day^{-1}$')
